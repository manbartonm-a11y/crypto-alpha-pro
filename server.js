  ...
  try {
    ...
    if (r.ok) {
      ...
      if (j.transactions?.[0]) {
        ...
        const usd = (t.amount_usd / 100000000).toFixed(1);
        lastWhale = `WHALE ALERT $${usd}M BTC ? ${t.to.owner || t.to.address.slice(0,10)}...`;
      }
    }
  } catch(e) {}
}, 30000);
...
app.get('/telegram', async (req, res) => {
  ...
  const isPremium = userId === "777000"; // ? change this to your Telegram ID when paid
  ...
  res.write("<style>body{margin:0;background:#000;color:#0f0;font-family:monospace;text-align:center;padding:20px}h1{color:#0ff;font-size:3.5em}.p{font-size:5.5em;color:#0f9;margin:10px}canvas{width:95%;max-width:600px;height:280px;border:6px solid #0f0;border-radius:20px;margin:30px auto;background:#000}.blur{filter:blur(12px);pointer-events:none}.premium{background:#0f0;color:#000;padding:20px 40px;font-size:2em;border-radius:20px;cursor:pointer;margin:30px auto;width:90%;max-width:500px}</style></head><body>");
  ...
  // THIS LINE IS NOW FIXED — BLUR ONLY FOR NON-PREMIUM
  ...
  if (!isPremium) {
    res.write("<div class='premium' onclick=\"location.href='https://t.me/CryptoBot?start=pay_to_YOUR_BOT_USERNAME'\">UNLOCK PREMIUM $9/month — Real Whales + Signals (Crypto)</div>");
  }
