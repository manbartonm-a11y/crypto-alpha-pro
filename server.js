...
app.get("/telegram", async (req, res) => {
  ...
  html += '<div style="font-size:2em;color:' + color + '">24h ' + (change >= 0 ? "+" : "") + change + "%</div>";
  html += '<canvas id="c"></canvas>';
  html += '<div style="background:#001a00;padding:20px;border:3px solid #0f0;border-radius:20px;margin:20px;font-size:1.5em' + (isPremium ? '' : ' class="blur"') + '">' + lastWhale + '</div>';
  html += '<div style="font-size:1.7em;color:#0f9">AI TRACKER Next pump in 4h 21m • Target: $112,000+</div>';
  html += isPremium ? '<div style="color:#0f9;font-size:2em">PREMIUM ACTIVE — Push alerts ON</div>' : '<div class="btn" onclick="location.href=\'https://t.me/CryptoBot?start=pay_to_crypto_alert_677_bot\'">Pay with Crypto (USDT/BTC/TON)</div><div class="btn" onclick="location.href=\'https://buy.stripe.com/00wdR92NcfZzdNgahlgEg00\'">Pay with Card / PayPal / Apple Pay</div>';
...
// WEBHOOK FOR UNLOCK AFTER PAYMENT (Stripe + CryptoBot)
app.post('/webhook', (req, res) => {
  const userId = req.body.user_id || "777000"; // from payment
