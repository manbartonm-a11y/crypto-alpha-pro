...
app.get('/telegram', (req,res)=>{
  ...
  if (isPremium && PORTFOLIOS[userId]) {
    let total = 0;
    PORTFOLIOS[userId].forEach(h=>total += (108420 - h.buy)*h.amount);
    pnl = <div style='background:#001a00;padding:30px;border:8px solid #0f0;border-radius:40px;margin:40px auto;max-width:700px;font-size:2.5em;color:'>
      Portfolio PnL: C:\Users\tulip\Desktop\programing crypto 1\proxy-server{Math.abs(total).toFixed(0)}
    </div>;
  }

  const paywall = isPremium ? pnl : 
...
canvas{width:96%;max-width:800px;height:460px;margin:40px auto;border:14px solid #0f0;border-radius:50px;background:#000;box-shadow:0 0 100px #0f0,0 0 150px #0f0}
...
<div class="p"></div>
<div style="font-size:4em;color:#0f0">24h </div>
<canvas id="c"></canvas>
...
ctx.stroke();
ctx.lineTo(780,460);ctx.lineTo(80,460);
