...
app.get('/telegram', async (req,res)=>{
  ...
  // Real price with strong fallback
  let price = 89600, change = "-2.8";
  try{
    const r = await fetch("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd&include_24hr_change=true",{
      headers:{"User-Agent":"CryptoAlphaPro/1.0"},timeout:6000});
...
<style>body{margin:0;background:#000;color:#0f0;font-family:monospace;text-align:center;padding:20px}h1{color:#0ff;font-size:3.5em}.p{font-size:5.5em;color:#0f9}canvas{width:95%;max-width:600px;height:280px;border:6px solid #0f0;border-radius:20px;margin:30px auto;background:#000}.blur{filter:blur(12px)}.btn{background:#0f0;color:#000;padding:20px 40px;font-size:2em;border-radius:20px;cursor:pointer;margin:20px auto;width:90%;max-width:500px}</style>
<script src="https://js.stripe.com/v3/"></script></head><body>
<h1>CRYPTO ALPHA PRO</h1>
<div class="p">$${price.toLocaleString()}</div>
...
<div class="btn" onclick="location.href='https://t.me/CryptoBot?start=pay_to_@crypto_alert_677_bot'">Pay with Crypto</div>
...
<script>const c=document.getElementById('c'),x=c.getContext('2d');c.width=600;c.height=280;x.fillStyle='#000';x.fillRect(0,0,600,280);x.strokeStyle='#0f0';x.lineWidth=8;x.beginPath();x.moveTo(0,250);for(let i=1;i<=12;i++)x.lineTo(i*50,Math.random()*100+150);x.stroke();x.fillStyle='rgba(0,255,0,0.3)';x.fill();</script>
</body></html>`;
  res.send(html);
});
