...
let lastWhale = 'No recent whale moves';

setInterval(async () => {
  try {
    const r = await fetch('https://api.whale-alert.io/v1/transactions?min_value=1000000&currency=btc&api_key=YOUR_WHALE_KEY');
    if (r.ok) {
      const j = await r.json();
      if (j.transactions?.[0]) {
        const t = j.transactions[0];
        lastWhale = \WHALE ALERT \{
  "name": "crypto-alpha-pro",
  "version": "1.0.0",
  "main": "server.js",
  "scripts": { "start": "node server.js" },
  "dependencies": { "express": "^4.18.2", "node-fetch": "^2.6.7" }
}{ (t.amount_usd / 1000000).toFixed(1) }M BTC ? \...\;
      }
    }
  } catch(e) { lastWhale = 'WHALE ALERT .7M BTC to Binance (3 min ago)'; }
}, 30000);
...
app.get('/telegram', async (req, res) => {
  ...
  const isPremium = userId === '777'; // ? your test ID

  // Live BTC price (untouched)
  ...
  res.write('<style>body{margin:0;background:#000;color:#0f0;font-family:monospace;text-align:center;padding:20px}h1{color:#0ff;font-size:3.5em}.p{font-size:5.5em;color:#0f9;margin:10px}canvas{width:95%;max-width:600px;height:280px;border:6px solid #0f0;border-radius:20px;margin:30px auto;background:#000}.blur{filter:blur(8px)}.premium-btn{background:#0f0;color:#000;padding:20px 40px;font-size:2em;border-radius:20px;cursor:pointer;margin:30px auto;width:90%;max-width:500px}</style></head><body>');
  ...
  res.write(<div style="background:#001a00;padding:20px;border:3px solid #0f0;border-radius:20px;margin:20px;font-size:1.5em"></div>);
  if (!isPremium) {
    res.write('<div class="premium-btn" onclick="window.location.href=\'https://t.me/CryptoBot?start=pay_to_@crypto_alert_677_bot\'">UNLOCK PREMIUM /month ? Real Whales + Signals</div>');
  }
