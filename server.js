  ...
  try {
    ...
    if (r.ok) {
      ...
      if (t) {
        ...
        const msg = "REAL WHALE ALERT " + btc + " BTC (~$" + Math.round(btc * 89000).toLocaleString() + "M) from " + t.sender.address.slice(0,10) + "... to " + t.receiver.address.slice(0,10) + "... just now";
        lastWhale = msg;
        sendPush(msg);
      }
    }
