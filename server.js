...
// Root fallback (prevents 404 on Render)
app.get("/", (req, res) => {
  res.send("<h1 style='color:#0f0;background:#000;text-align:center;padding:100px;font-family:monospace'>Crypto Alpha Pro LIVE on Render! ??<br><a href='/telegram' style='color:#0ff'>Open Dashboard</a></h1>");
});
...
<script>
  ...
  async function load(){
    try {
      ...
      document.getElementById("grok").textContent = "API rate limited — retry in 30s";
    }
  }
...
// Render-specific (binds to 0.0.0.0 and dynamic PORT)
