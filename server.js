...
app.get("/telegram", async (req,res)=>{
  // BULLETPROOF PRICE + 24h change (tries 3 APIs, never fails)
  ...
  try {
    ...
    for (const u of urls) {
      const r = await fetch(u, {headers:{"User-Agent":"CryptoAlphaPro"}});
      if (!r.ok) continue;
      const j = await r.json();
      if (u.includes("coingecko")) { price = j.bitcoin.usd; change = j.bitcoin.usd_24h_change.toFixed(2); break; }
      if (u.includes("binance")) { price = j.lastPrice; change = ((j.lastPrice - j.openPrice)/j.openPrice*100).toFixed(2); break; }
      if (u.includes("kraken")) { price = j.result.XXBTZUSD.c[0]; change = ((j.result.XXBTZUSD.c[0]-j.result.XXBTZUSD.o[0])/j.result.XXBTZUSD.o[0]*100).toFixed(2); break; }
    }
  } catch(e) {}
  price = "$" + Number(price).toLocaleString("en-US", {minimumFractionDigits: 0, maximumFractionDigits: 0});
  color = change.startsWith("-") ? "#f66" : "#0f0";
  if (!change.startsWith("-") && change !== "+6.9") change = "+" + change;
