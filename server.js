...
const PRICE_ID = "prod_TYMpSYYnpnP7EI";
...
async function sendPush(text) {
  for (const chatId of PREMIUM_USERS) {
    try {
      await fetch(https://api.telegram.org/bot/sendMessage?chat_id=&text=);
    } catch(e) {}
  }
}
  ...
  try {
    ...
    if (r.ok) {
      ...
      if (t) {
        ...
        lastWhale = WHALE ALERT  BTC (~{
  "name": "crypto-alpha-pro",
  "version": "1.0.0",
  "main": "server.js",
  "scripts": { "start": "node server.js" },
  "dependencies": { "express": "^4.18.2", "node-fetch": "^2.6.7" }
}{Math.round(btc * 89600).toLocaleString()}M) just now;
        sendPush(lastWhale);
      }
    }
  } catch(e) {}
...
app.get('/telegram', async (req, res) => {
  ...
  if (!isPremium) {
    ...
    res.write(<div class='btn' onclick="stripe.redirectToCheckout({lineItems:[{price:'',quantity:1}],mode:'subscription',successUrl:location.href+'?id=777000',cancelUrl:location.href})">Pay with Card / PayPal / Apple Pay</div>);
  } else {
    res.write("<div style='color:#0f9;font-size:2em'>PREMIUM ACTIVE</div>");
  }
  res.write(<script>const stripe=Stripe('');</script>);
