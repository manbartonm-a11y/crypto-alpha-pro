  ...
  try {
    ...
    if (r.ok) {
      ...
      if (j.transactions && j.transactions[0]) {
        ...
        const usd = (t.amount_usd / 1000000).toFixed(1);
...
app.get('/telegram', async (req, res) => {
  const isPremium = false; // ? will be true after payment

  // Live price (untouched)
  ...
  res.write("<style>body{margin:0;background:#000;color:#0f0;font-family:monospace;text-align:center;padding:20px}h1{color:#0ff;font-size:3.5em}.p{font-size:5.5em;color:#0f9;margin:10px}canvas{width:95%;max-width:600px;height:280px;border:6px solid #0f0;border-radius:20px;margin:30px auto;background:#000}.blur{filter:blur(10px);pointer-events:none}.premium{background:#0f0;color:#000;padding:20px 40px;font-size:2em;border-radius:20px;cursor:pointer;margin:30px auto;width:90%;max-width:500px}</style></head><body>");
