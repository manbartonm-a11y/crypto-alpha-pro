...
// In-memory DB (works on Render free tier)
const PORTFOLIOS = {};     // {userId: [{coin, amount, buyPrice}]}
const PREMIUM = new Set(); // user IDs with premium

app.get("/", (req,res)=>res.send("<h1 style=\"color:#0f0;background:#000;text-align:center;padding:200px;font-family:monospace\">CRYPTO ALPHA PRO LIVE!<br><a href=\"/telegram?id=123\" style=\"color:#0ff;font-size:50px\">OPEN DASHBOARD</a></h1>"));
...
app.get("/telegram", async (req,res)=>{
  ...
  if (isPremium && PORTFOLIOS[userId]) {
    ...
    for (const h of PORTFOLIOS[userId]) {
      try {
        const r = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${h.coin}&vs_currencies=usd`);
        const j = await r.json();
        const cur = j[h.coin]?.usd || h.buyPrice;
        total += (cur - h.buyPrice) * h.amount;
      } catch(e) {}
    }
    pnlText = total>=0 ?
      `<div class="box" style="color:#0f0;font-size:2em">Portfolio PnL: +$${total.toFixed(2)}</div>` :
      `<div class="box" style="color:#f66;font-size:2em">Portfolio PnL: $${total.toFixed(2)}</div>`;
  }

  const premiumContent = isPremium ? pnlText : 
    `<a href="https://t.me/CryptoBot?start=pay_to_@crypto_alert_677_bot" class="btn">UNLOCK PREMIUM ? $9/month</a>`;

  res.send(`
<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Crypto Alpha Pro</title><script src="https://telegram.org/js/telegram-web-app.js"></script>
<style>
...
canvas.width=750;canvas.height=420;ctx.fillStyle="#000";ctx.fillRect(0,0,750,420);
...
app.post("/add", (req,res)=>{
  ...
  PREMIUM.add(id); // remove this line later when real payments are active
  res.send("OK");
});
...
app.listen(process.env.PORT || 10000, "0.0.0.0", ()=>console.log("STEP 3 COMPLETE — MONEY READY"));
